...
class Maze:
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        ...
        """
        valid_waypoints = self.get_valid_waypoints()[cur_shape]
        x, y = cur_waypoint
        distances = [
                (waypoint, euclidean_distance(cur_waypoint, waypoint)) 
                for waypoint in valid_waypoints 
                if waypoint != cur_waypoint and self.is_valid_move((x, y, cur_shape), (*waypoint, cur_shape))
            ]

        # print(f"Valid waypoints: {valid_waypoints}")
        # print(f"Distances: {distances}")

        distances.sort(key=lambda x: x[1])

        nearest_neighbors = [waypoint for waypoint, dist in distances[:self.k]]
    ...
    def is_valid_move(self, start, end):
        ...
        """
        start_x, start_y, start_shape = start
        end_x, end_y, end_shape = end
        if end_shape not in [0,1,2]:
            return False
        # 创建两个位置的外星人
        alien_start = self.create_new_alien(*start)
        if start_shape != end_shape:
            alien_end = self.create_new_alien(*end)
